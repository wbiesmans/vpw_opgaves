...
def solve_1_knapsack_01(capacity: int, items: list[Item]):
    ...
    for current_item_plus_one_index in range(1, number_of_items + 1):
        ...
        for current_capacity in range(capacity + 1):
            ...
            if current_item_requirement <= current_capacity:
                dp[current_item_plus_one_index][current_capacity] = max(
                    dp[current_item_plus_one_index - 1][current_capacity],
                    current_item.value
                    + dp[current_item_plus_one_index - 1][
                        current_capacity - current_item_requirement
                    ],
                )
            else:
                dp[current_item_plus_one_index][current_capacity] = dp[
                    current_item_plus_one_index - 1
                ][current_capacity]
    ...
    for current_item_plus_one_index in range(number_of_items, 0, -1):
        if (
            dp[current_item_plus_one_index][current_capacity]
            == dp[current_item_plus_one_index - 1][current_capacity]
        ):
            continue
...
def solve_2_knapsack_01(capacity: tuple[int], items: list[Item]):
    ...
    # initialize zeroes
    dp = [
        [[0] * (capacity[1] + 1) for _ in range(capacity[0] + 1)]
    ...
    for current_item_plus_one_index in range(1, number_of_items + 1):
        ...
        for current_capacity_0 in range(capacity[0] + 1):
            for current_capacity_1 in range(capacity[1] + 1):
                ...
                if (
                    current_item_requirement_0 <= current_capacity_0
                    and current_item_requirement_1 <= current_capacity_1
                ):
                    take = (
                        current_item.value
                        + dp[current_item_plus_one_index - 1][
                            current_capacity_0 - current_item_requirement_0
                        ][current_capacity_1 - current_item_requirement_1]
                    )
                    dp[current_item_plus_one_index][current_capacity_0][current_capacity_1] = max(
                        skip, take
                    )
                else:
    ...
    for current_item_plus_one_index in range(number_of_items, 0, -1):
        if (
            dp[current_item_plus_one_index][current_capacity_0][current_capacity_1]
            == dp[current_item_plus_one_index - 1][current_capacity_0][current_capacity_1]
        ):
            continue
        item = items[current_item_plus_one_index - 1]
        items_to_return += (item,)
...
def solve_n_knapsack_01(
    capacities: tuple[int],
    items: list[Item],
    item_index: int = 0,
    memo: typing.Union[None, dict[tuple[int, tuple[int]]]] = None,
) -> tuple[int, tuple[Item]]:
    if memo is None:
        memo = {}
    ...
    result_weight, result_items = solve_n_knapsack_01(
        tuple(capacities), items, item_index + 1, memo
    )

    # Option 2: take current item
    current_item = items[item_index]
    fits_in_knapsack = all(
        requirement <= current_capacity
        for current_capacity, requirement in zip(capacities, current_item.requirements)
    )

    result_items_with = None
    if fits_in_knapsack:
        new_capacities = tuple(
            current_capacity - requirement
            for current_capacity, requirement in zip(capacities, current_item.requirements)
        )
        result_weight_with, result_items_with = solve_n_knapsack_01(
            new_capacities, items, item_index + 1, memo
        )
...
if __name__ == "__main__":
    ...
    for entry in range(num_entries):
        ...
        # Start of parsing
        num_dishes, max_weight = tuple(map(int, input_lines[line_idx].strip().split()))
        line_idx += 1

        dishes = []
        for i in range(num_dishes):
            weight, initial_satisfaction, delta_satisfaction = tuple(
                map(int, input_lines[line_idx].strip().split())
            )
            line_idx += 1
            dishes.append(Item(initial_satisfaction, (weight,)))
            satisfaction = initial_satisfaction
            num_duplicates = max_weight // weight - 1
            for i in range(num_duplicates):
                satisfaction -= delta_satisfaction
                dishes.append(Item(satisfaction, (weight,)))

        # Start of processing
        value = solve_1_knapsack_01(max_weight, dishes)
        solution_str = f"{entry + 1} {value[0]}"
