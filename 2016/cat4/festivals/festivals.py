...
class Graph:
    ...
    def state_space_search(
        self, end: str, frontier: tuple[float, list[Path]] = None
    ) -> tuple[int, Path]:
        """
        ...
        while frontier:
            ...
            if most_promising_path.nodes[-1] == end:
                if all(
                    [
                        mandatory_edge in most_promising_path.edges
                        for mandatory_edge in self.edges
                        if mandatory_edge.mandatory
                    ]
                ):
...
def process(graph: Graph, start: str, end: str) -> typing.Union[int, str]:
...
if __name__ == "__main__":
    ...
    for i in range(number_of_entries):
        ...
        number_of_concerts = int(input_lines[line_index].strip())

        line_index += 1
        # Start of parsing
        concerts = []
        for j in range(number_of_concerts):
            start_hour, start_min, end_hour, end_min = tuple(
                map(int, input_lines[line_index].strip().split())
            )
            line_index += 1
            start = 60 * start_hour + start_min
            end = 60 * end_hour + end_min
            concerts.append(tuple([start, end]))

        edges = []
        for idx_from, prev_concert in enumerate(concerts):
            edges.append(Edge("start", idx_from, 0, bidirectional=False))
            for idx_to, next_concert in enumerate(concerts):
                if prev_concert[1] <= next_concert[0]:
                    edges.append(Edge(idx_from, idx_to, -1, bidirectional=False))
            edges.append(Edge(idx_from, "end", 0, bidirectional=False))
        ...
        # Start of processing
        value = process(graph, start, end)
        print(f"{i + 1} {abs(value)+ 1}")
        if output_filename:
            with open(output_filename, "a") as f:
                f.write(f"{i + 1} {abs(value)+ 1}\n")
        # End of processing
